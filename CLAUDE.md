# Claude Code Project Instructions — winapi-kotlin

## Project Overview

This is **winapi-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`winapi`](https://crates.io/crates/winapi).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/winapi/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/winapi>
